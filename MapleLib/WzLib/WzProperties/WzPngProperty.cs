...
namespace MapleLib.WzLib.WzProperties
{
    ...
    public class WzPngProperty : WzImageProperty
    {
        ...
        public byte[] Decompress(byte[] compressedBuffer, int decompressedSize)
        {
        ...
        public uint calcLineSum()
        {
            ...
            switch (format + format2)
            {
                case 1:
                    ...
                    decBuf = new byte[uncompressedSize];
                    zlib.Read(decBuf, 0, uncompressedSize);
                    byte[][] multiArr1 = Database.Convert1DArray2D(decBuf, height, width * 2);
                    for(int i = height; i > 0; i--)
                    {
                        linesum = PixelCLB.Packets.CRC32.CalcCRC32(multiArr1[i-1])^linesum;
                    }
                    return linesum;
                case 2:
                    bmp = new Bitmap(width, height, PixelFormat.Format32bppArgb);
                    ...
                    uncompressedSize = width * height * 4;
                    decBuf = new byte[uncompressedSize];
                    byte[][] multiArr2 = Database.Convert1DArray2D(decBuf, height, width * 2);
                    for (int i = height; i > 0; i--)
                    {
                        linesum = PixelCLB.Packets.CRC32.CalcCRC32(multiArr2[i - 1]) ^ linesum;
                    }
                    return linesum;
                case 513:
                    uncompressedSize = width * height * 2;
                    decBuf = new byte[uncompressedSize];
                    byte[][] multiArr513 = Database.Convert1DArray2D(decBuf, height, width * 2);
                    for (int i = height; i > 0; i--)
                    {
                        linesum = PixelCLB.Packets.CRC32.CalcCRC32(multiArr513[i - 1]) ^ linesum;
                    }
                    return linesum;
                case 517:
                    bmp = new Bitmap(width, height);
                    uncompressedSize = width * height / 128;
                    decBuf = new byte[uncompressedSize];
                    byte[][] multiArr517 = Database.Convert1DArray2D(decBuf, height, width * 2);
                    for (int i = height; i > 0; i--)
                    {
                        linesum = PixelCLB.Packets.CRC32.CalcCRC32(multiArr517[i - 1]) ^ linesum;
                    }
                    return linesum;
                default:
                    Helpers.ErrorLogger.Log(Helpers.ErrorLevel.MissingFeature, string.Format("Unknown PNG format {0} {1}", format, format2));
